...
impl Keypair {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, ed25519_dalek::SignatureError> {
        let bytes = <[u8; ed25519_dalek::KEYPAIR_LENGTH]>::try_from(bytes)
            .map_err(|_| ed25519_dalek::SignatureError::from_source(String::from(
                "candidate keypair byte array is not the correct length",
            )))?;
